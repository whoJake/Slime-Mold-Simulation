...
static color repulseColor;
final color agentColor = color(0, 255, 255);

final static int strokeSize = 15;
final static float evaporateSpeed = 5;
final static float diffusionRate = 0.2;
    ...
    for(int offsetX = -1; offsetX <= 1; offsetX++){
       for(int offsetY = -1; offsetY <= 1; offsetY++){
           ...
           if(sampleX >= 0 && sampleX < width && sampleY >= 0 && sampleY < height){
              ...
              if(isSameColor(c, repulseColor)) continue;
              sumR += red(c);
              sumG += green(c);
              sumB += blue(c);
           }
...
float sense(Agent a, float sensorOffset){
    ...
    for(int offsetX = -1; offsetX <= 1; offsetX++){
       for(int offsetY = -1; offsetY <= 1; offsetY++){
           ...
           if(isSameColor(sample, repulseColor)){
              sensorSum -= 30000;
              continue;
           }
...
void spawn_inwardCircle(){
   for(int i = 0; i < numOfAgents; i++){
      ...
      agents[i] = new Agent(width/2 + cos(ang) * dist, height/2 + sin(ang) * dist, ang + 180);
   }
}
