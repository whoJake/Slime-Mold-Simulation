...
final static float circleSpawnRadius = 100;

static color wallColor;
color drawColor = color(220);
color agentColor = color(255, 150, 150);
color bgColor = color(0);

final static int simWidth = 500;
final static int simHeight = 500;

static int strokeSize = 5;
static float evaporateSpeed = 1;
static float diffusionRate = 0.8;
static float sensorOffsetDistance = 14;
static float sensorAngle = 100;
static float turnSpeed = 4;


static int restartNumOfAgents = 9000;
static int numOfAgents = 9000;
...
void initSettings(){
   ...
   allButtons[6] = new Button(520, 420, 70, 30, "Wall");
   allButtons[5] = new Button(615, 420, 70, 30, "Color");
   allButtons[7] = new Button(710, 420, 70, 30, "Attract");
   ...
   allText[9] = new Text(520, 475, "Stroke Size", false);
   
   allSliders[0] = new Slider(640, 195, 100, 0, 1, diffusionRate);
   allSliders[1] = new Slider(640, 225, 100, 0, 10, evaporateSpeed);
   allSliders[2] = new Slider(640, 255, 100, 2, 52, sensorOffsetDistance);
   allSliders[3] = new Slider(640, 285, 100, 0, 100, sensorAngle);
   allSliders[4] = new Slider(640, 315, 100, 0, 100, turnSpeed);
   
   allSliders[5] = new Slider(520, 400, 200, 1000, 50000, numOfAgents);
   
   allSliders[6] = new Slider(640, 470, 100, 2, 30, strokeSize);
}
...
void settingsUpdate(){
   ...
   if(!locked){
     ...
     if(pressedButton == allButtons[6]){
        wallToggle = true;
     }
     if(pressedButton == allButtons[7]){
        wallToggle = false; 
     }
