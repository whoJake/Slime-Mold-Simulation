...
final static float sensorOffsetDistance = 20;
...
float sense(Agent a, float sensorOffset){
    ...
    for(int offsetX = -1; offsetX <= 1; offsetX++){
       for(int offsetY = -1; offsetY <= 1; offsetY++){
           ...
           sensorSum += red(get(samplePosX, samplePosY));
       }
    }
    return sensorSum;
}
...
void drawAgent(Agent a){
  set(int(a.posX), int(a.posY), color(255, 0, 0));
}
...
void spawn_circle(){
   for(int i = 0; i < numOfAgents; i++){
      ...
      agents[i] = new Agent(maxOffsetX * random(1) + width/2, maxOffsetY * random(1) + height/2, 180 - randomAngle);
   }
}
